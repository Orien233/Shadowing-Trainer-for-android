...
const OrtApi *get_compatible_ort_api_or_throw(const char *component) {
  ...
  for (int api_version = 1; api_version <= kMaxProbeApiVersion; ++api_version) {
    const OrtApi *candidate = ort_api_base->GetApi(static_cast<uint32_t>(api_version));
...
void validate_required_ort_api_or_throw(const OrtApi *ort_api,
  ...
  if (ort_api == nullptr) {
    throw std::runtime_error(std::string(component) + ": ORT API pointer is null");
  }
...
#define CHECK_ORT_FN(name)                                                   \
  ...
  CHECK_ORT_FN(CreateSession)
  CHECK_ORT_FN(CreateSessionFromArray)
  CHECK_ORT_FN(CreateTensorWithDataAsOrtValue)
  CHECK_ORT_FN(Run)
  CHECK_ORT_FN(GetTypeInfo)
  CHECK_ORT_FN(CastTypeInfoToTensorInfo)
  CHECK_ORT_FN(GetTensorElementType)
  CHECK_ORT_FN(GetDimensionsCount)
  CHECK_ORT_FN(GetDimensions)
  CHECK_ORT_FN(GetTensorTypeAndShape)
  CHECK_ORT_FN(GetTensorMutableData)
  CHECK_ORT_FN(SessionGetInputCount)
  CHECK_ORT_FN(SessionGetInputName)
  CHECK_ORT_FN(SessionGetInputTypeInfo)
  CHECK_ORT_FN(SessionGetOutputCount)
  CHECK_ORT_FN(SessionGetOutputName)
  CHECK_ORT_FN(SessionGetOutputTypeInfo)
  CHECK_ORT_FN(ReleaseTensorTypeAndShapeInfo)
...
void apply_session_config_entry_if_supported(const OrtApi *ort_api,
                                             OrtSessionOptions *session_options,
                                             const char *key, const char *value) {
  if (ort_api->AddSessionConfigEntry == nullptr) {
    LOGF("MoonshineModel: AddSessionConfigEntry unsupported, skipping '%s'", key);
    return;
  }
  throw_on_ort_error(ort_api, ort_api->AddSessionConfigEntry(session_options, key, value),
                     "AddSessionConfigEntry failed");
}
}  // namespace
...
MoonshineModel::MoonshineModel(bool log_ort_run, float max_tokens_per_second)
    : ort_api(nullptr),
      ort_env(nullptr),
      ort_session_options(nullptr),
      ort_memory_info(nullptr),
      ort_session_allocator(nullptr),
      ort_string_allocator(nullptr),
      encoder_session(nullptr),
      decoder_session(nullptr),
      tokenizer(nullptr),
      max_tokens_per_second(max_tokens_per_second),
      log_ort_run(log_ort_run) {
  ort_api = get_compatible_ort_api_or_throw("MoonshineModel");
  validate_required_ort_api_or_throw(ort_api, "MoonshineModel");
  ...
  if (ort_memory_info == nullptr) {
    throw std::runtime_error("ORT memory info is null");
  }
  ...
  throw_on_ort_error(ort_api, ort_api->CreateSessionOptions(&ort_session_options),
                     "CreateSessionOptions failed");
  if (ort_session_options == nullptr) {
    throw std::runtime_error("ORT session options are null");
  }
  throw_on_ort_error(ort_api,
                     ort_api->SetSessionGraphOptimizationLevel(
                         ort_session_options, ORT_ENABLE_EXTENDED),
                     "SetSessionGraphOptimizationLevel failed");
  apply_session_config_entry_if_supported(
      ort_api, ort_session_options, "session.load_model_format", "ORT");
  apply_session_config_entry_if_supported(
      ort_api, ort_session_options, "session.use_ort_model_bytes_directly", "1");
  apply_session_config_entry_if_supported(
      ort_api, ort_session_options, "session.use_env_allocators", "1");
  apply_session_config_entry_if_supported(
      ort_api, ort_session_options, "session.disable_prepacking", "1");
  if (ort_api->DisableCpuMemArena != nullptr) {
    throw_on_ort_error(ort_api, ort_api->DisableCpuMemArena(ort_session_options),
                       "DisableCpuMemArena failed");
  } else {
    LOG("MoonshineModel: DisableCpuMemArena unsupported, skipping");
  }
}
...
MoonshineModel::~MoonshineModel() {
  if (ort_api != nullptr) {
    ...
    if (ort_session_options) ort_api->ReleaseSessionOptions(ort_session_options);
    if (ort_memory_info) ort_api->ReleaseMemoryInfo(ort_memory_info);
