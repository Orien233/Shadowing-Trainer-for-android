...
static void throw_on_ort_error(const OrtApi *ort_api, OrtStatus *status,
                               const char *context) {
  if (status == nullptr) {
    return;
  }
...
static const OrtApi *get_compatible_ort_api_or_throw(const char *component) {
...
static void validate_required_ort_api_or_throw(const OrtApi *ort_api,
...
MoonshineStreamingModel::MoonshineStreamingModel(bool log_ort_run)
  ...
  ort_api = get_compatible_ort_api_or_throw("MoonshineStreamingModel");
  validate_required_ort_api_or_throw(ort_api, "MoonshineStreamingModel");
